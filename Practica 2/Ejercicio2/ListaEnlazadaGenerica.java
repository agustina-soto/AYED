...
public class ListaEnlazadaGenerica<T> extends ListaGenerica<T> {
	...
	/* Agrega todos los elementos del arreglo que recibe como parámetro 
	 * y retornar true si todos ellos fueron agregados.*/
	public boolean agregar(T[] arreglo) {
		for (T dato : arreglo) {
			if (!this.agregarFinal(dato)) { // Si no puedo agregar
				return false;
			}
		}
		return true;
